...
import { URL,ScreenKey,TokenContext } from '../globals/constants'
import bgImage from '../../../image/loginadmin.jpg'
...
export default function App(props) {
    ...
    const changeRefesh=useContext(TokenContext).changeRefesh;
    const senddata = async () => {
        ...
        if (res.status === 200) {
            ...
            console.log("RESULT ",result.infoUser);
            setSpinner(false);
            storeData(result.token, result.infoUser);
            changeRefesh();
            props.navigation.navigate(ScreenKey.TabHomeProfile);
